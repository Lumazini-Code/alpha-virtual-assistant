...
def main_window():
    ...
    sg.theme('darkblue1')
    layout = [
        [sg.Text('olá, Bem vindo a Alpha', right_click_menu= (True, 'GLP'), font='arial')],
        [sg.Image('Alpha.Icon.png')],
        [sg.Text('Use palavras-chave para fazer a pergunta , EX: busque, encontre, ', font='arial')],
        [sg.Text('ache entre outros', font='arial')],
        [sg.Button(key= 'Ligar Alpha' , button_text='Ligar Alpha' , border_width= 0, font='arial'),sg.Button('Fechar Alpha', border_width= 0, font='arial'), sg.Button('Configurações', border_width= 0, font='arial')],
        ]
    
    return sg.Window('AVA  Alpha - Tela Inicial', layout= layout, finalize= True)
...
def config_window():
    ...
    layout2 = [
        [sg.Text('escreva abaixo por oque que você quer que a alpha te chame', font='arial')],
        [sg.Input(key= 'nameuser'), sg.Button('salvar', border_width= 0, font='arial')],
        [sg.Button('Salvar E Voltar', border_width= 0, font='arial')]
    ]

    return sg.Window('AVA  Alpha - Configurações', layout= layout2, finalize= True)
...
while True:
    ...
    if window == janela_inicial and event == 'Ligar Alpha':
        ...
        with open('ask.dll',  "r") as bleu:
            pergunta = bleu.read()
        sg.popup(f'{username}: {pergunta} \n \n Alpha: \n {ablablabla}', title= 'Resposta', auto_close_duration= 20, auto_close= True)
