...
#
# Nomes confirmados contra tools/file_tools.py (register_all): list_files,
# read_file, write_file, str_replace, run_command, stat — todos registrados
# juntos por register_file_tools(), por isso ficam todos na categoria "file".
#
# search/patch/git/test/symbol permanecem INFERIDOS — os módulos
# search_tools.py, patch_tools.py, git_tools.py, test_tools.py e
# symbol_lookup.py (registrados separadamente em alpha_code.py) não foram
# enviados para conferência. Se os nomes reais divergirem, `_tools_for_kind`
# cai de volta para "enviar tudo" (ver fallback abaixo), então nada quebra —
# mas vale corrigir aqui assim que esses arquivos forem enviados.
# ════════════════════════════════════════════════════════════════════════════

TOOL_CATEGORIES: dict[str, tuple[str, ...]] = {
    "file": ("list_files", "read_file", "write_file", "str_replace", "run_command", "stat"),
    "search": ("search_code", "grep", "semantic_search"),
    "patch": ("apply_patch",),
    "test": ("run_tests",),
    "symbol": ("symbol_lookup", "find_references", "find_definition"),
}
...
2. FAÇA MUDANÇAS MÍNIMAS. Prefira str_replace / apply_patch a write_file.
...
class Agent:
    ...
    async def run(self, task: str) -> AsyncGenerator[Event, None]:
        ...
        try:
            for step_idx in range(self.max_steps):
                ...
                # 2c. Model router
                choice = pick_model(step_ctx, override=self.model_override)
                yield Event(
                ...
                # 2d. Chama LLM (com retry externo para 429 persistente)
                message = None
                model_used = ""
                fallback = False
                llm_retries = 3
                for llm_attempt in range(llm_retries):
                    try:
                        message, model_used, fallback = await self._call_llm(
                            choice=choice,
                            step_idx=step_idx,
                            kind=kind,
                        )
                        ...
                        err_msg = str(e)
                        is_rate_limit = "429" in err_msg or "rate-limited" in err_msg or "rate limit" in err_msg.lower()
                        if is_rate_limit and llm_attempt < llm_retries - 1:
                            wait_s = 30.0 * (llm_attempt + 1)
                            log.warning(
                                f"LLM 429 persistente (tentativa {llm_attempt+1}/{llm_retries}). "
                                f"Esperando {wait_s}s antes de retry."
                            )
                            yield Event(
                                event=EventType.ERROR,
                                data={
                                    "error": f"Groq rate-limited, esperando {wait_s}s (tentativa {llm_attempt+1}/{llm_retries})",
                                    "fatal": False,
                ...
                if tool_calls:
                    ...
                    for tc in tool_calls:
                        ...
                        result = await self.registry.dispatch(call, ctx_for_tool)

                        # track file changes
                        if call.name in ("write_file", "str_replace", "apply_patch"):
    ...
    async def _call_llm(self, choice: ModelChoice, step_idx: int, kind: str = "editing") -> tuple[dict, str, bool]:
        ...
        if estimated_tokens > safe_limit:
            ...
            if estimated_tokens > safe_limit:
                best_model, best_limit = max(safe_limits.items(), key=lambda kv: kv[1])
        ...
        if result.get("too_large"):
            ...
            try:
                ...
                log.exception(f"Emergency summarization failed: {e}")
    ...
    async def _do_llm_request(self, choice: ModelChoice, step_idx: int, kind: str = "editing") -> dict:
        """Faz uma chamada HTTP real ao /chat/tools. Retorna dict com message/model/fallback_used/too_large."""
        ...
        if r.status_code == 503:
            raise RuntimeError(f"LLM indisponível (503): {r.text[:300]}")
        if r.status_code >= 400:
            raise RuntimeError(f"LLM /chat/tools → {r.status_code}: {r.text[:300]}")
